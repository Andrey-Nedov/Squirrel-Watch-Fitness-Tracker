...
Устройство базируется на AVR-микроконтроллере Atmega328p в компактном SMD-корпусе. Синхронизацию со смартфоном обеспечивает встренный в часы bluetooth-модуль. Информация о пройденном пути берётся с GPS-модуля смартфона. Так же в устройстве предусмотрен вибромотор для обратной связи.
